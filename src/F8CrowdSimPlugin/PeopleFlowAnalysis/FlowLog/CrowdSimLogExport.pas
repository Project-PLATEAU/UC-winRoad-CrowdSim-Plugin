...
procedure TLogCrossSectionFlow.CollectCSFLog(const SimBegTime, CurrTime, BeginTime: TDateTime);
    var
        ...
        SARange  : double;
    begin
    if not Assigned(p_LogDataList) then
        Exit;
...
procedure TLogCrossSectionFlow.CollectData(const controller: F8CrowdSimControllerClass);
    ...
    if MovingFeatureListClass.numberOfMovingFeatures > 0 then
        ...
        for var i := 0 to MovingFeatureListClass.numberOfMovingFeatures - 1 do
            begin
            if MovingFeatureListClass.movingFeature[i].numberOfFocusedHistories < 2 then
                continue;

            chara := MovingFeatureListClass.movingFeature[i].myInstance;
            if not Assigned(chara) then
                continue;

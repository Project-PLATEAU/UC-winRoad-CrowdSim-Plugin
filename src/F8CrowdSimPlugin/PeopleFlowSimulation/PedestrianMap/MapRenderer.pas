...
procedure TMapRenderer.Initialize(const aData: VertexDataArray; CellSize: Double);
    var
        HeightData    : HeightResultArrayType;
        i, j: Integer;
        minH: Single;
        terrainHeight : single;
    begin
    minH := HEIGHT_FROM_ZERO;
    ...
    for i := 0 to Length(p_Data) - 1 do
        begin
        HeightData := theApplicationServices.project.GetHeightsAt(p_Data[i].X, p_Data[i].Z, [_hTerrain, _hRoad, _hInterSection]);

        if Length(HeightData) = 0 then
            terrainHeight := HEIGHT_FROM_ZERO
        else
            begin
            for j := 0 to Length(HeightData) - 1 do
                begin
                if j = 0 then
                    begin
                    minH := HeightData[j].hHeight;
                    Continue;
                    end;

                minH := Min(minH, HeightData[j].hHeight);
                end;
            terrainHeight := minH;
            end;

        p_vertices[i][0] := p_Data[i].X;
        p_vertices[i][1] := terrainHeight + 0.2;
...
procedure TMapRenderer.BindVertexBuffer;
    ...
    glBufferData(GL_ARRAY_BUFFER, Length(p_vertices) * 3 * sizeof(GLFloat), @p_vertices[0], GL_STATIC_DRAW);

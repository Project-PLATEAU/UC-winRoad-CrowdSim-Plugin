...
type
    ...
    TPedestrianCellGroup = class
        ...
        public
            ...
            // MENGE出力
            procedure ExportObstacleXML(const aParentNode : IXMLNode; const aRoundedCorner: Boolean = True); virtual;
            // PluginData入出力
...
procedure TPedestrianCellGroup.ExportObstacleXML(const aParentNode : IXMLNode; const aRoundedCorner: Boolean = True);
    const
        TOP_LEFT_CORNER    : EdgePositionTypeSet = [EdgePositionType._Top, EdgePositionType._Left];
        TOP_RIGHT_CORNER   : EdgePositionTypeSet = [EdgePositionType._Top, EdgePositionType._Right];
        BOTTOM_LEFT_CORNER : EdgePositionTypeSet = [EdgePositionType._Bottom, EdgePositionType._Left];
        BOTTOM_RIGHT_CORNER: EdgePositionTypeSet = [EdgePositionType._Bottom, EdgePositionType._Right];
    var
        exclusionList: TDictionary<Integer, Boolean>;

    procedure ExtractRoundEdge(var aList: TList<TParametricLine2D>);
        function  IsCornerCell(const aOutlines: EdgePositionTypeSet; const aKey: Integer; out aEdge: TParametricLine2D): Boolean;
            begin
            Result := False;

            if  aOutlines = TOP_LEFT_CORNER then
                begin
                aEdge := ConvertToDiagonal_LeftTop(CellTable[aKey].AreaLocal);
                Result := True;
                Exit;
                end;

            if aOutlines = TOP_RIGHT_CORNER then
                begin
                aEdge := ConvertToDiagonal_RightTop(CellTable[aKey].AreaLocal);
                Result := True;
                Exit;
                end;

            if aOutlines = BOTTOM_LEFT_CORNER then
                begin
                aEdge := ConvertToDiagonal_LeftBottom(CellTable[aKey].AreaLocal);
                Result := True;
                Exit;
                end;

            if aOutlines = BOTTOM_RIGHT_CORNER then
                begin
                aEdge := ConvertToDiagonal_RightBottom(CellTable[aKey].AreaLocal);
                Result := True;
                Exit;
                end
            end;

        procedure AddUniqueEdge(const aValue: TParametricLine2D);
            begin
            if aList.Contains(aValue) then
                Exit;
        ...
        var
            key: Integer;
            keyList: TList<Integer>;
            e: TParametricLine2D;
            outlines: EdgePositionTypeSet;
        begin
        // グループの内外周辺を抽出する
        // 各セルごとに、4辺がグループ内の他のセルと接続しているか調べる。接続していない辺が内外周辺
        if not Assigned(aList) then
            Exit;
        // CellID順にソートする
        // これにより左上が先頭に来る
        keyList := TList<Integer>.Create;
        try
            keyList.AddRange(CellTable.Keys.ToArray);
            keyList.Sort(
            ...
            for key in keyList do
                ...
                outlines := [];
                if not CellTable.ContainsKey(CellTable[key].CellID.AboveID) then
                    outlines := outlines + [EdgePositionType._Top];

                if not CellTable.ContainsKey(CellTable[key].CellID.LeftID) then
                    outlines := outlines + [EdgePositionType._Left];

                if not CellTable.ContainsKey(CellTable[key].CellID.BelowID) then
                    outlines := outlines + [EdgePositionType._Bottom];

                if not CellTable.ContainsKey(CellTable[key].CellID.RightID) then
                    outlines := outlines + [EdgePositionType._Right];

                // 特定の2辺のみの場合は対角線を出す
                if IsCornerCell(outlines, key, e) then
                    AddUniqueEdge(e)
                else
                    begin
                    // 上->左->下->右順に探索
                    if EdgePositionType._Top in outlines then
                        AddUniqueEdge(ConvertToEdge(CellTable[key].AreaLocal, EdgePositionType._Top));

                    if EdgePositionType._Left in outlines then
                        AddUniqueEdge(ConvertToEdge(CellTable[key].AreaLocal, EdgePositionType._Left));

                    if EdgePositionType._Bottom in outlines then
                        AddUniqueEdge(ConvertToEdge(CellTable[key].AreaLocal, EdgePositionType._Bottom));

                    if EdgePositionType._Right in outlines then
                        AddUniqueEdge(ConvertToEdge(CellTable[key].AreaLocal, EdgePositionType._Right));
                    end;
                end;
        finally
            FreeAndNil(keyList);
    ...
    procedure ExtractEdge(var aList: TList<TParametricLine2D>);
        procedure AddUniqueEdge(const aValue: TParametricLine2D);
            begin
            if aList.Contains(aValue) then
                Exit;
    ...
    function  FindPreviousIndex(const aCurrent: Integer): Integer;
        var
            prev: Integer;
        begin
        Result := 0;
        ...
        while 0 < prev do
            begin
            if not exclusionList[prev] then
                begin
                Result := prev;
                Exit;
                end;
    ...
    function  FindNextIndex(const aCurrent: Integer): Integer;
        var
            nex: Integer;
        begin
        Result := exclusionList.Keys.Count - 1;
        nex := aCurrent + 1;
        if exclusionList.Keys.Count <= nex then
            Exit;

        while nex < exclusionList.Keys.Count - 1 do
            begin
            if not exclusionList[nex] then
                begin
                Result := nex;
                Exit;
                end;
    ...
    var
        itemNode: IXMLNode;
        edgeList, outputEdgeList: TList<TParametricLine2D>;
        outputList: TList<TPoint3D>;
        i: Integer;
        findNew: Boolean;
        current, next, previous: Integer;
        vtx: IXMLNode;
    begin
    edgeList       := TList<TParametricLine2D>.Create;
    outputEdgeList := TList<TParametricLine2D>.Create;
    outputList     := TList<TPoint3D>.Create;
    exclusionList  := TDictionary<Integer, Boolean>.Create;
    try
        if aRoundedCorner then
            ExtractRoundEdge(edgeList)
        else
            ExtractEdge(edgeList);

        // 辺のローカル座標を取得する
        // 結果が一筆書きになるようソートする
        // 辺は外周辺だけとは限らないのでリストが空になるまで続ける
        while edgeList.Count > 0 do
            begin
            outputEdgeList.Clear;
            outputList.Clear;
            exclusionList.Clear;
            outputEdgeList.Add(edgeList.ExtractAt(0));
            findNew := True;
            ...
            while findNew do
                ...
                for i := 0 to edgeList.Count - 1 do
                    begin
                    if outputEdgeList.Last.IsConnected_OnlyTailHead(edgeList[i]) then
                        begin
                        outputEdgeList.Add(edgeList.ExtractAt(i));
                        findNew := True;
                        Break;
                        end
                    else if outputEdgeList.Last.IsConnected_OnlyTailTail(edgeList[i]) then
                        begin
                        outputEdgeList.Add(edgeList.ExtractAt(i).Inverse);
                        findNew := True;
                        Break;
                        end;
                    end;
            ...
            // 重複している点を削除
            // 除外リストを初期化
            for i := 0 to outputEdgeList.Count - 1 do
                begin
                outputList.Add(outputEdgeList[i].a);
                exclusionList.Add(outputList.Count - 1, False);
                end;

            Assert(outputList.Count >= 4);
            // 点数削減処理: 直線となる辺の中間点を削除
            // →3点ずつ見て縦横どちらかに直線なら中間点を除外リストに登録(iを中間としてi-1, i+1を見る)
            // →中間点を削除するので始点、終点は削除されない
            for i := 1 to outputList.Count - 2 do
                begin
                current  := i;
                previous := FindPreviousIndex(current);
                next     := FindNextIndex(current);

                if (outputList[current] - outputList[previous]).Normalise = (outputList[next] - outputList[current]).Normalise then
                    exclusionList[current] := True;
                end;

            // 全てチェックしたら除外リストに登録されている頂点を削除
            for i := outputList.Count - 1 downto 0 do

                begin
                if exclusionList[i] then
                    outputList.Delete(i);
                end;

            // XML出力
            itemNode := aParentNode.AddChild(TAG_OBSTACLE);
            itemNode.Attributes[TAG_CLOSED] := VALUE_CLOSED;

            for i := 0 to outputList.Count - 1 do
                begin
                vtx := itemNode.AddChild(TAG_VERTEX);
                vtx.Attributes[TAG_VERTEX_X] := outputList[i].X;
                vtx.Attributes[TAG_VERTEX_Y] := outputList[i].Z;
                end;
            end;
    finally
        FreeAndNil(edgeList);
        FreeAndNil(outputEdgeList);
        FreeAndNil(outputList);
        FreeAndNil(exclusionList);
        end;
    end;

...
type
    ...
    TPedestrianCrossWalk = class(TPedestrianCellGroup)
        ...
        public
            ...
            // MENGE出力用
            procedure ExportSignaledObstacleXML(const aParentNode: IXMLNode; const aElapsedTimeSecond: Integer);
            // PluginData入出力
    ...
    TPedestrianCrossWalkList = class
        ...
        public
            ...
            procedure ExportObstacleXML(const aParentNode: IXMLNode; const aElapsedTimeSecond: Integer);
...
procedure TPedestrianCrossWalk.ExportSignaledObstacleXML(const aParentNode: IXMLNode; const aElapsedTimeSecond: Integer);
    begin
    if _IsEditing then
        begin
        EndEditCrossWalk;
    ...
    if GetCurrentSignal(aElapsedTimeSecond) = PedestrianAreaCrossingSignal._pacsCrossingGreen then
        Exit;

    ExportObstacleXML(aParentNode, False);
    end;
...
procedure TPedestrianCrossWalkList.ExportObstacleXML(const aParentNode: IXMLNode; const aElapsedTimeSecond: Integer);
    var
        crossWalk: TPedestrianCrossWalk;
    begin
    for crossWalk in p_List do
        crossWalk.ExportSignaledObstacleXML(aParentNode, aElapsedTimeSecond);
    end;

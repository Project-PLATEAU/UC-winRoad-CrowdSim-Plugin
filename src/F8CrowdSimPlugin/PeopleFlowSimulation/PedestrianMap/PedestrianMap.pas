...
type
    ...
    TPedestrianMap = class
        ...
        public
            ...
            procedure ExportObstacleXML(var rootNode : IXMLNode; const aElapsedTimeSecond: Integer);
...
procedure TPedestrianMap.OverlayRoad(const aHoriConv: IF8WrHorizontalCoordinateConvertor; const aRoad: TTranRoad);
    ...
    function  FindRoadCornerCellID: Boolean;
        ...
        // 最初はメッシュ外判定なし
        Result := False;
        area := aRoad.BoundingArea;
        findLT := RequireCellInfo(ConvertLatLonToLocal(area[VertexPositionType._LeftTop]), cellIDLT, status, diffLT);
         // (左上)道路とメッシュは全く重なっていない
        if IsRightOfMesh(diffLT) or IsBelowOfMesh(diffLT) then
            Exit;
        findRB := RequireCellInfo(ConvertLatLonToLocal(area[VertexPositionType._RightBottom]), cellIDRB, status, diffRB);
         // (右下)道路とメッシュは全く重なっていない
        if IsLeftOfMesh(diffRB) or IsAboveOfMesh(diffRB) then
            Exit;
        findRT := RequireCellInfo(ConvertLatLonToLocal(area[VertexPositionType._RightTop]), cellIDRT, status, diffRT);
        findLB := RequireCellInfo(ConvertLatLonToLocal(area[VertexPositionType._LeftBottom]), cellIDLB, status, diffLB);
        ...
        if findLT and findRT and findRB and findLB then
            begin
            // メッシュ内
            roadCorner[VertexPositionType._LeftTop] := cellIDLT;
            roadCorner[VertexPositionType._RightTop] := cellIDRT;
            roadCorner[VertexPositionType._LeftBottom] := cellIDLB;
            roadCorner[VertexPositionType._RightBottom] := cellIDRB;
            end
        else if findLB and findRB then
            begin
            // 上
            roadCorner[VertexPositionType._LeftTop] := p_CellList[GetColumnIndex(diffLT)].CellID;
            roadCorner[VertexPositionType._RightTop]  := p_CellList[GetColumnIndex(diffRT)].CellID;
            roadCorner[VertexPositionType._LeftBottom] := cellIDLB;
            roadCorner[VertexPositionType._RightBottom] := cellIDRB;
            end
            ...
            roadCorner[VertexPositionType._LeftTop] := CellIDLT;
            roadCorner[VertexPositionType._RightTop]  := CellIDRT;
            roadCorner[VertexPositionType._LeftBottom] := p_CellList[(Config.RowCount - 1) * Config.ColumnCount + GetColumnIndex(diffLB)].CellID;
            roadCorner[VertexPositionType._RightBottom] := p_CellList[(Config.RowCount - 1) * Config.ColumnCount + GetColumnIndex(diffRB)].CellID;
            end
        else if findRT and findRB then
            begin
            // 左
            roadCorner[VertexPositionType._LeftTop] := p_CellList[GetRowIndex(diffLT) * Config.ColumnCount].CellID;
            roadCorner[VertexPositionType._RightTop]  := CellIDRT;
            roadCorner[VertexPositionType._LeftBottom] := p_CellList[GetRowIndex(diffLB) * Config.ColumnCount].CellID;
            roadCorner[VertexPositionType._RightBottom] := cellIDRB;
            end
            ...
            // 右
            roadCorner[VertexPositionType._LeftTop] := CellIDLT;
            roadCorner[VertexPositionType._RightTop] := p_CellList[GetRowIndex(diffRT) * Config.ColumnCount + (Config.ColumnCount - 1)].CellID;
            roadCorner[VertexPositionType._LeftBottom] := cellIDLB;
            roadCorner[VertexPositionType._RightBottom] := p_CellList[GetRowIndex(diffRB) * Config.ColumnCount + (Config.ColumnCount - 1)].CellID;
            end
        else if findRB then
            begin
            // 左上
            roadCorner[VertexPositionType._LeftTop] := p_CellList[0].CellID;
            roadCorner[VertexPositionType._RightTop]  := p_CellList[GetColumnIndex(diffRT)].CellID;
            roadCorner[VertexPositionType._LeftBottom] := p_CellList[GetRowIndex(diffLB) * Config.ColumnCount].CellID;
            roadCorner[VertexPositionType._RightBottom] := cellIDRB;
            end
            ...
            // 右上
            roadCorner[VertexPositionType._LeftTop] := p_CellList[GetColumnIndex(diffLT)].CellID;
            roadCorner[VertexPositionType._RightTop]  := p_CellList[Config.ColumnCount - 1].CellID;
            roadCorner[VertexPositionType._LeftBottom] := cellIDLB;
            roadCorner[VertexPositionType._RightBottom] := p_CellList[GetRowIndex(diffRB) * Config.ColumnCount + (Config.ColumnCount - 1)].CellID;
            end
        else if findRT then
            ...
            roadCorner[VertexPositionType._LeftTop] := p_CellList[GetRowIndex(diffLT) * Config.ColumnCount].CellID;
            roadCorner[VertexPositionType._RightTop]  := CellIDRT;
            roadCorner[VertexPositionType._LeftBottom] := p_CellList[(Config.RowCount - 1) * Config.ColumnCount].CellID;
            roadCorner[VertexPositionType._RightBottom] := p_CellList[(Config.RowCount - 1) * Config.ColumnCount + GetColumnIndex(diffRB)].CellID;
            end
        else if findLT then
            begin
            ...
            roadCorner[VertexPositionType._RightTop]  := p_CellList[GetRowIndex(diffRT) * Config.ColumnCount + (Config.ColumnCount - 1)].CellID;
            roadCorner[VertexPositionType._LeftBottom] := p_CellList[(Config.RowCount - 1) * Config.ColumnCount + GetColumnIndex(diffLB)].CellID;
            roadCorner[VertexPositionType._RightBottom] := p_CellList[Config.AllCellCount - 1].CellID;
            end
        else
            begin
    ...
    function  FindNextCell(const aCellID: TCellID): Integer;
        begin
        // 道路のBoundingAreaと重なっている次のマスを検索する
        // 右端 or Not
        if aCellID.ColumnIdx = roadCorner[VertexPositionType._RightBottom].ColumnIdx then
            begin
            // 最後
            if aCellID.ID = roadCorner[VertexPositionType._RightBottom].ID then
                begin
                Result := aCellID.ID + 1;
                Exit;
                end;
            // 1行下の左下
            Result := (aCellID.RowIdx + 1) * Config.ColumnCount + (roadCorner[VertexPositionType._LeftBottom].ColumnIdx - 1);
            end
        else
    ...
    p_CellListCS.Enter;
    try
        if not FindRoadCornerCellID then
            Exit;
        ...
        while i <= roadCorner[VertexPositionType._RightBottom].ID do
            ...
            i := FindNextCell(p_CellList[i].CellID);
            end;
    finally
        p_CellListCS.Leave;
        end;
...
procedure TPedestrianMap.ExportObstacleXML(var rootNode : IXMLNode; const aElapsedTimeSecond: Integer);
    const
        TAG_OBSTACLE_SET = 'ObstacleSet';
        TAG_TYPE         = 'type';
        TAG_CLASS        = 'class';
        VALUE_TYPE       = 'explicit';
        VALUE_CLASS      = '1';
    var
        obstacleSet: IXMLNode;
    begin
    obstacleSet := rootNode.AddChild(TAG_OBSTACLE_SET);
    obstacleSet.Attributes[TAG_TYPE] := VALUE_TYPE;
    obstacleSet.Attributes[TAG_CLASS] := VALUE_CLASS;

    p_CrossWalks.ExportObstacleXML(obstacleSet, aElapsedTimeSecond);
    p_NoEntrySet.ExportObstacleXML(obstacleSet);
    end;
            ...
            for idxRow := 0 to rowCount - 1 do
                ...
                for idxColumn := 0 to columnCount - 1 do
                    ...
                    if idGenerator.Generate(idxCell, newID) then
                        ...
                        cellCenter := aMapOrigin.MoveTo(southOffset + cellSize / 2, eastOffset + cellSize / 2).Local;
                        p_CellList.Add(TPedestrianCell.Create(newID, vertexs, vertexsLocal, vertexsGL, cellCenter, status));
                        // 描画用データの作成
...
procedure TNavMeshBlock.AfterConstruction;
    ...
    if Odd(p_List.Count) then
        p_RepresentativePoint := p_List[p_List.Count div 2].CenterLocal
    else
        p_RepresentativePoint := p_List[(p_List.Count div 2) - 1].CenterLocal;
    end;

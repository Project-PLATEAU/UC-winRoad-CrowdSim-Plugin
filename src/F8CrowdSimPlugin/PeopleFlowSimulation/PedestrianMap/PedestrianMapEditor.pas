...
procedure TPedestrianMapEditor.UpdateUnderMouseGuide(const aGLPosition: TPoint3D);
    ...
    if p_EditMap.CrossWalks.IsEditing then
        p_EditingGuide.UpdateMouseAreaGuide(p_API, p_EditMap, localPosition, CrosswalkEditSize)
    else
        p_EditingGuide.UpdateMouseAreaGuide(p_API, p_EditMap, localPosition, StatusEditSize);
    end;

...
type
    ...
    TimeAndPositionType = record
        ...
        procedure ConvertFromPacket(const time: TDateTime; const adata: AgentPacketDataType);
        end;
    ...
    MovingFeatureClass = class
        ...
        public
            ...
            procedure   SetThePositionByMENGE(const initialTime: TDateTime; const agentData: AgentPacketDataType);
            procedure   AfterCalculateByMENGE;
            procedure   EndSimulationFromMenge;
    ...
    MovingFeatureListClass = class
        ...
        public
            ...
            class procedure SetThePositionFromMENGE(const initialTime: TDateTime; const dataArray: TArrayAgentPacketDataType);
            class procedure EndSimulationFromMenge;
            ...
            class procedure ExportMFJson(const path: string; expInterval: double; api: IF8ApplicationServices);
...
class procedure MovingFeatureListClass.EndSimulationFromMenge;
    var
        i : Integer;
    begin
    for i := 0 to p_list.Count-1 do
        begin
        p_list[i].EndSimulationFromMenge;
        end;
    ...
    if Assigned(p_Maglist) then
        ...
        for I := 0 to p_Maglist.Count-1 do
            begin
            p_Maglist[i].EndSimulationFromMenge;
            end;
        end;
    end;

class procedure MovingFeatureListClass.ExportMFJson(const path: string; expInterval: double; api: IF8ApplicationServices);
    var
        JsonSerializer : TJsonSerializer;
        ExportHelper   : TMFJsonExportHelper;
        Writer         : TStreamWriter;
        Json           : string;
        exList         : TObjectList<MovingFeatureClass>;
    begin
    ExportHelper   := TMFJsonExportHelper.Create;
    JsonSerializer := TJsonSerializer.Create;
    exList         := TObjectList<MovingFeatureClass>.Create;
    exList.AddRange(p_list);
    if Assigned(p_Maglist) then
        exList.AddRange(p_Maglist);
...
class procedure MovingFeatureListClass.SetThePositionFromMENGE(const initialTime: TDateTime; const dataArray: TArrayAgentPacketDataType);
    var
        i   : Integer;
    begin
    Assert(Length(dataArray) <= numberOfMovingFeatures, 'invalid packet count');
    for i := Low(dataArray) to High(dataArray) do
        begin
        movingFeature[dataArray[i].classI].SetThePositionByMENGE(initialTime, dataArray[i]);
        end;
    end;
...
procedure MovingFeatureClass.AfterCalculateByMENGE;
    begin
    if Assigned(p_temporalHistories) and (p_temporalHistories.Count > 0) then
        begin
        if not Assigned(p_focusedHistories) then
            p_focusedHistories := Tlist<TimeAndPositionType>.Create;
        p_focusedHistories.AddRange(p_temporalHistories);
...
procedure MovingFeatureClass.CheckAndSetCharacter(const aproject: IF8ProjectForRoad; const options: MFJsonLoaderOptionsClass);
    ...
    if Assigned(ageProp) and Assigned(genderProp) then
        ...
        p_character := options.unknown;
        p_characterInRain := options.unknown;
        end;
...
procedure MovingFeatureClass.EndSimulationFromMenge;
    var
        i   : Integer;
        gp  : GLPointType;
        dn,de : double;
    begin
    dn := theApplicationServices.distanceNorth;
    de := theApplicationServices.distanceEast;
    for i := 0 to p_focusedHistories.Count - 1 do
        begin
        gp := p_focusedHistories[i].position;
        if (InRange(gp[_x], 0,de) and
            InRange(gp[_z], 0,dn)) then
            begin
            p_focusedHistories.DeleteRange(0,i);
            break;
            end;
        if i = p_focusedHistories.Count-1 then
            begin
            p_focusedHistories.Clear;
            end;
        end;

    for i := p_focusedHistories.Count - 1 downto 0 do
        begin
        gp := p_focusedHistories[i].position;
        if (InRange(gp[_x], 0,de) and
            InRange(gp[_z], 0,dn)) then
            begin
            p_focusedHistories.DeleteRange(i,p_focusedHistories.Count-i);
            break;
            end;
        end;
    ...
    if p_focusedHistories.Count>0  then
        p_startPoint := p_focusedHistories[0].position;
    if p_focusedHistories.Count>(p_focusedHistories.Count-i) then
        p_goalPoint  := p_focusedHistories[p_focusedHistories.Count-i].position;
    end;
...
procedure MovingFeatureClass.SetThePositionByMENGE(const initialTime: TDateTime; const agentData: AgentPacketDataType);
    const
        TIME_STEP = 0.1;
    var
        tp  : TimeAndPositionType;
        SecondsToAdd: Double;
    begin
    if not Assigned(p_temporalHistories) then
        p_temporalHistories := TList<TimeAndPositionType>.Create;
    SecondsToAdd := p_temporalHistories.Count * TIME_STEP / 86400;
    tp.ConvertFromPacket(initialTime + SecondsToAdd, agentData);
    p_temporalHistories.Add(tp);

    p_lastStateIndex := agentData.state;
    end;
...
procedure TimeAndPositionType.ConvertFromPacket(const time: TDateTime; const adata: AgentPacketDataType);
    var
        DebugMessage : String;
    begin
    self.time := time;
    self.position := GLPointFrom(adata.pos.x, adata.pos.y, adata.pos.z);

    DebugMessage := PWideChar(self.position[_x].ToString+', '+self.position[_y].ToString+', '+self.position[_z].ToString);
    end;

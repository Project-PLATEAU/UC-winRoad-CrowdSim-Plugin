...
type
    ...
    TTranRoad = class
        private
            ...
            procedure Make(const aCityRoad: IXMLRoadType);
...
/// </summary>
procedure TTranRoad.Make(const aCityRoad: IXMLRoadType);
    const
    ...
    procedure MakeLOD1PedestrianArea;
        ...
        multiSurfaceMem := multiSurface.MultiSurface.SurfaceMember;
        ...
        for i := 0 to multiSurfaceMem.Count - 1 do
            begin
            if not Supports(multiSurfaceMem.Items[i]._Surface, IXMLPolygonType, pol) then
                Continue;
            ...
            p_PedestrianArea.Last.ImportSingleSurface(lil.PosList.Text);
            end;
        end;
    ...
    procedure MakeLOD3PedestrianArea;
        ...
        for i := 0 to aCityRoad.TrafficArea.Count - 1 do
            ...
            try
                multiSurfaceMem := traf.Lod3MultiSurface.MultiSurface.SurfaceMember;

                for j := 0 to multiSurfaceMem.Count - 1 do
                    begin
                    if not Supports(multiSurfaceMem.Items[j]._Surface, IXMLPolygonType, pol) then
                        Continue;
                    ...
                    strList.Add(lil.PosList.Text);
                    end;
            p_PedestrianArea.Last.ImportMultiSurface(strList);
            finally
                FreeAndNil(strList);
                end;
            end;
